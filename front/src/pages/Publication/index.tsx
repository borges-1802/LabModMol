...
const Publication: React.FC = () => {
    ...
    const authors = [
        { name: 'Carlos Rangel', key: 'carlos', publications: publicationsByYear },
        { name: 'Alessandra', key: 'alessandra', publications: {
            '2025': [
        ...
        { name: 'Bárbara', key: 'barbara', publications: {
            '2025': [
                { title: 'Thiourea as a Promising Scaffold for Development of Agents Toward TriTryp Diseases.', link: 'http://dx.doi.org/10.1002/slct.202404210' },
    ...
    return (
        <>
            ...
            <Container>
                ...
                {authors.map((author) => (
                    <div key={author.key} style={{ marginBottom: '3rem' }}>
                        <Title style={{ fontSize: '2rem', marginBottom: '1.5rem' }}>{author.name}</Title>
                        {Object.keys(author.publications).length > 0 ? (
                            Object.keys(author.publications).sort((a, b) => Number(b) - Number(a)).map((year) => (
                                <YearSection key={year}>
                                    ...
                                    <PublicationsGrid>
                                        {author.publications[year].map((pub, index) => (
                                            <PublicationCard
                                                key={index}
                                                title={pub.title}
                                                link={pub.link}
                            ...
                            <Description>Nenhuma publicação cadastrada.</Description>
                        )}
                    </div>
                ))}
                {/* Fonte removida */}
